...
The package includes built-in knowledge bases for common entity inference:

- **Gene Symbol → Ensembl ID**: Common genes (BRAF, TP53, IL6, etc.)
- **Disease Name → EFO/MONDO**: Common diseases (cancer types, metabolic disorders)
- **Drug Name → CHEMBL ID**: Common drugs (imatinib, trastuzumab, etc.)

These are used by the `AdaptiveRegexExtractor` to improve coverage without API calls.

## Performance Characteristics

| Mode | Cost | Speed | Coverage |
|------|------|-------|----------|
| Regex only | Free | ~1-2 sec | 60-70% |
| Regex + LLM | ~$0.10-0.50/run | ~30-60 sec | 85-95% |

*Coverage estimates based on typical GitHub issue repositories*

## Testing

```bash
# Run tests with coverage
pytest --cov=ot_miner tests/

# Run specific test
pytest tests/test_extractors.py::test_regex_extraction
```
